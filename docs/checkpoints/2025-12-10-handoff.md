...
| Function | Purpose |
|----------|---------|
| `normalizeBreakpoint` | Convert string → BreakpointRule object |
...
**Rationale**: Enables independent testing without mocking, avoids `segmenter.ts` → `breakpoint-utils.ts` → `segmenter.ts` cycle.

### 2. Pre-computed Data Structures

All lookups are now O(1):
- `pageIdToIndex: Map<number, number>` - page ID → array index
...
```
✅ applyBreakpoints: 160 → 60 (62.5% reduction)
✅ findBreakPosition: 47 → 26 (extracted helpers)
⚠️ segmentPages: 35 (acceptable for main entry point)
```
...
| Topic | File |
|-------|------|
| Main entry point | `src/segmentation/segmenter.ts` → `segmentPages()` |
| Breakpoint post-processing | `src/segmentation/segmenter.ts` → `applyBreakpoints()` |
