...
cd /Users/rhaq/workspace/flappa-doormal
...
**The Issue**: Tests use `maxPages: 1` with `breakpoints: ['{{tarqim}}\\s*', '']`. Page 1 content has `(٥٤١ - ٦٢٠ هـ)` which contains `)` (a tarqim punctuation), causing mid-page splits.
...
- Chapter markers (باب، فصل، كتاب)
- Hadith numbers (numbered patterns like ٧٥٦٣ -)
...
rules: [
    {
        ...
        occurrence: 'last',
        split: 'after',
        maxSpan: 1,           // ← This forced per-page splitting
        fallback: 'page'       // ← Fall back to page boundary if no match
    }
]
```
...
3. Example: "فصل: Topic." would split at the period, creating a tiny "." segment
...
3. **Pattern examples**:
   - ✅ Correct string: `breakpoints: ['{{tarqim}}\\s*', '']`
   - ✅ Correct regex literal: `expect(result[0].content).toMatch(/\.\s*$/)`
   - ❌ Wrong: `breakpoints: ['{{tarqim}}\s*', '']` (not escaped)
   - ❌ Wrong: `/\\.\\s*$/` (double-escaped in regex literal)
...
- Fallback patterns (period → newline → page boundary)
...
**Specific Issue**: Page 1 of book 2588 contains `(٥٤١ - ٦٢٠ هـ)` which has `)` - a tarqim punctuation mark. With `maxPages: 1`, breakpoints finds this parenthesis and splits mid-page, causing the test expectation (`endsWith: 'الرياض'`) to fail.

**Migration Done**:
1. ✅ Updated `data` type declaration to include `breakpoints`, `maxPages`, `prefer` (line 36-42)
2. ✅ Removed tarqim rule from inline rules arrays (both 2576 and 2588)
3. ✅ Added `breakpoints: ['{{tarqim}}\\s*', '']`, `maxPages: 1`, `prefer: 'longer' as const` to inline data
4. ✅ Updated `segmentPages` calls to pass all options
...
Since we don't have exact page→character mapping, we estimate using average characters per page with 50% padding to ensure the window is large enough.
...
| `{{basmalah}}` | "بِسْمِ اللَّهِ" patterns |
| `{{bab}}` | "باب" (chapter) patterns |
| `{{fasl}}` | "فصل" (section) patterns |
| `{{kitab}}` | "كتاب" (book) patterns |
...
| Test Suite | Status |
|------------|--------|
| `segmenter.test.ts` - breakpoints (13 tests) | ✅ PASSING |
| `segmenter.test.ts` - other tests | ✅ PASSING |
| `index.test.ts` - 2576 | ❌ FAILING |
| `index.test.ts` - 2588 | ❌ FAILING |
...
| Topic | Where to Look |
|-------|---------------|
| Core implementation | `src/segmentation/segmenter.ts` → `applyBreakpoints()` function (lines ~432-576) |
| API types/options | `src/segmentation/types.ts` → `SegmentationOptions` interface |
| Unit tests | `src/segmentation/segmenter.test.ts` → search for `describe('breakpoints'` |
| Integration with segmentPages | `src/segmentation/segmenter.ts` → end of `segmentPages()` function |

### If Asked About Segmentation Rules
| Topic | Where to Look |
|-------|---------------|
| Rule processing logic | `src/segmentation/segmenter.ts` → `segmentPages()` function |
| Rule type definitions | `src/segmentation/types.ts` → `SplitRule` interface |
| lineStartsWith/lineStartsAfter | `src/segmentation/segmenter.ts` → search for these property names |
| Template matching | `src/segmentation/segmenter.ts` → search for `template` |
| fuzzy matching | `src/segmentation/fuzzy.ts` |

### If Asked About Tokens ({{tarqim}}, {{basmalah}}, etc.)
| Topic | Where to Look |
|-------|---------------|
| Token definitions | `src/segmentation/tokens.ts` → `TOKEN_PATTERNS` object |
| Token expansion | `src/segmentation/tokens.ts` → `expandTokens()` function |
...
| Topic | Where to Look |
|-------|---------------|
| Integration test data | `src/index.test.ts` → inline `data = { pages, rules, ... }` objects |
| Integration test JSON (unused) | `test/2576.json`, `test/2588.json` - **NOTE: tests use inline data, not these files** |
| Breakpoints unit tests | `src/segmentation/segmenter.test.ts` → `describe('breakpoints'` block |
| Other segmenter tests | `src/segmentation/segmenter.test.ts` |

### If Asked About the "Premature Split" Bug (Original Problem)
| Topic | Where to Look |
|-------|---------------|
| Problem description | This document → "Historical Context: The Old maxSpan System" section |
| Solution | `applyBreakpoints()` post-processing approach |
| Test proving fix | `segmenter.test.ts` → `'should NOT create tiny segments from punctuation within chapter titles'` |

### If Asked About Page/Content Mapping
| Topic | Where to Look |
|-------|---------------|
| Page type definition | `src/segmentation/types.ts` → `Page` interface |
| Segment type definition | `src/segmentation/types.ts` → `Segment` interface |
| Content window calculation | `src/segmentation/segmenter.ts` → `applyBreakpoints()` → look for `approxWindowEnd` |

### If Asked About Arabic Text Processing
| Topic | Where to Look |
|-------|---------------|
| HTML to markdown conversion | `src/index.test.ts` → `htmlToMarkdown()` helper function |
| Fuzzy Arabic matching | `src/segmentation/fuzzy.ts` |
| Arabic punctuation patterns | `src/segmentation/tokens.ts` → `tarqim` token |
| Arabic numeral patterns | `src/segmentation/tokens.ts` → `raqm`, `raqms` tokens |
...
| Topic | Where to Look |
|-------|---------------|
| Adding new structural rules | `src/segmentation/types.ts` → `SplitRule` for type, `segmenter.ts` for implementation |
| Adding new tokens | `src/segmentation/tokens.ts` → add to `TOKEN_PATTERNS` |
| Adding new segment types | `src/segmentation/types.ts` → modify `Segment`, `SegmentMeta` interfaces |
